...
M3815_exps = trip_tib_2000 %>%
    filter(replicate == "M3814_Rep1", ssODN == "ssODN") %>% 
  distinct(replicate, exp_pool, ssODN) %>% pull(exp_pool)
trip_tib_c5_m3814 <- trip_tib_2000 %>%
  filter(exp_pool %in% M3815_exps)

c5_siNT_exp = trip_tib_2000 %>%  
  filter(siRNA == "siNT", replicate %in% c("clonesiRNARep1", "clonesiRNARep2"), 
         plasmid == "LBR2") %>% 
  distinct(replicate, exp_pool) %>% pull(exp_pool)

trip_tib_c5_siNT <- trip_tib_2000 %>%
  filter(exp_pool %in% c5_siNT_exp)


pool_SSTR_exp = trip_tib_2000 %>%  
    filter(ssODN == "ssODN", cell_line == "RSTP2_2000", 
           plasmid == "LBR2", replicate != "mean", time == 64) %>% 
    distinct(replicate, exp_pool) %>% pull(exp_pool)

trip_tib_SSTR <- trip_tib_2000 %>%
  filter(exp_pool %in% pool_SSTR_exp)
...
trip_tib_pathways <- trip_tib_c5_m3814 %>%
...
trip_tib_pathways <- trip_tib_c5_siNT %>%
  gather(NHEJ, MMEJ, other_indels, SSTR, key = "pathway", value = 'ratios') %>%
  distinct(barcode, exp, pathway, .keep_all = TRUE)
...
```{r efficiency and balance per BC in replicates of M3814 ssODN experiments, fig.height=8, fig.width=14}
pathway_distribution <- trip_tib_c5_m3814 %>%
  filter(exp_pool == "indel_M3814_Rep1_clone5_LBR2_DMSO_ssODN_72") %>%
...
barcode_order <- trip_tib_c5_m3814 %>%
  filter(exp_pool == "indel_M3814_Rep1_clone5_LBR2_DMSO_ssODN_72") %>%
...
p1b <- ggplot(pathway_distribution,
  ...
  xlab("IPR") +
  scale_fill_manual("legend", values = colores) +
  ggtitle("Pathway contribution per barcode")  + theme(legend.position = "none")


## 1 uM
pathway_distribution <- trip_tib_c5_m3814 %>%
  filter(exp_pool == "indel_M3814_Rep1_clone5_LBR2_M3814_1uM_ssODN_72") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
p2b <- ggplot(pathway_distribution,
       aes(barcode, ratios, width=1)) +
  geom_bar(stat = "identity", aes(fill = pathway)) +
  theme_bw(base_size = 16) +
  theme(axis.text.x=element_blank(),
...
## 2 uM
pathway_distribution <- trip_tib_c5_m3814 %>%
  filter(exp_pool == "indel_M3814_Rep1_clone5_LBR2_M3814_2uM_ssODN_72") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
p3b <- ggplot(pathway_distribution,
       aes(barcode, ratios, width=1)) +
  geom_bar(stat = "identity", aes(fill = pathway)) +
  theme_bw(base_size = 16) +
  theme(axis.text.x=element_blank(),
...
## 4 uM
pathway_distribution <- trip_tib_c5_m3814 %>%
  filter(exp_pool == "indel_M3814_Rep1_clone5_LBR2_M3814_4uM_ssODN_72") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
p4b <- ggplot(pathway_distribution,
       aes(barcode, ratios, width=1)) +
  geom_bar(stat = "identity", aes(fill = pathway)) +
  theme_bw(base_size = 16) +
  theme(axis.text.x=element_blank(),
...
plot_grid(p1e, p2e, p3e, p4e, p1b,p2b,p3b,  p4b, nrow = 2)
```

```{r efficiency and balance per BC in replicates of siRNA NT ssODN experiments, fig.height=8, fig.width=14}
# DMSO rep1
pathway_distribution <- trip_tib_c5_siNT %>%
  filter(exp_pool == "indel_clonesiRNARep1_clone5_LBR2_DMSO_siNT_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
barcode_order <- trip_tib_c5_siNT %>%
  filter(exp_pool == "indel_clonesiRNARep1_clone5_LBR2_DMSO_siNT_64") %>%
  distinct(efficiency, barcode)  %>%
  arrange(dplyr::desc(efficiency)) %>%
  dplyr::select(barcode)
...
## NU7441 rep 1
pathway_distribution <- trip_tib_c5_siNT %>%
  filter(exp_pool == "indel_clonesiRNARep1_clone5_LBR2_NU7441_siNT_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
## DMSO rep 2
pathway_distribution <- trip_tib_c5_siNT %>%
  filter(exp_pool == "indel_clonesiRNARep2_clone5_LBR2_DMSO_siNT_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
## NU7441 rep 2
pathway_distribution <- trip_tib_c5_siNT %>%
  filter(exp_pool == "indel_clonesiRNARep2_clone5_LBR2_NU7441_siNT_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
```{r efficiency and balance per BC in replicates of ssODN experiments in the pool, fig.height=8, fig.width=14}
# DMSO rep1



pathway_distribution <- trip_tib_SSTR %>%
  filter(exp_pool == "indel_LBR2ssODNRep1_LBR2_DMSO_ssODN_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
barcode_order <- trip_tib_SSTR %>%
  filter(exp_pool == "indel_LBR2ssODNRep1_LBR2_DMSO_ssODN_64") %>%
  distinct(SSTR, barcode)  %>%
  arrange(dplyr::desc(SSTR)) %>%
  dplyr::select(barcode)
...
p1e <- trip_tib_SSTR %>%
  filter(exp_pool == "indel_LBR2ssODNRep1_LBR2_DMSO_ssODN_64") %>%
  distinct(efficiency, barcode, SSTR)  %>% 
  ggplot(.,
       aes(reorder(barcode, -efficiency), efficiency, width=1)) +
  geom_bar(stat = "identity") +
  theme_bw(base_size = 16) +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
...
## NU7441 rep 1
pathway_distribution <- trip_tib_SSTR %>%
  filter(exp_pool == "indel_LBR2ssODNRep1_LBR2_NU7441_ssODN_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
## DMSO rep 2
pathway_distribution <- trip_tib_SSTR %>%
  filter(exp_pool == "indel_LBR2ssODNRep2_LBR2_DMSO_ssODN_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
## NU7441 rep 2
pathway_distribution <- trip_tib_SSTR %>%
  filter(exp_pool == "indel_LBR2ssODNRep2_LBR2_NU7441_ssODN_64") %>%
  gather(NHEJ, MMEJ, SSTR, other_indels,
         key = "pathway", value = 'ratios') %>%
    distinct(barcode, pathway, ratios) %>%
  group_by(barcode) %>%
...
ggplot(trip_tib_SSTR,
       aes(replicate, SSTR, color = replicate)) +
  geom_quasirandom() +
  theme_bw(base_size = 16) +
  facet_grid(. ~ drug)
```
...
# Pool data
Let's see how the blance of the other pathways change if we split the data on high and low SSTR in seperate replicates. 
...
#                    domain_levels, 'SSTR', min_count = 25, y_plabel=c(0.5, 0.7))

# plot_comb_grid_boxplot(eff.dt, opt_df, domain_levels, 'SSTR', group_by='condition', min_count = 25, color=c("DMSO" = "black", "NU7441" = "gray"))
# 
# plot_comb_grid(indel.dt[exp_pool=='mean_RSTP2_2000_LBR2_NU7441_ssODN_64'], opt_df,
#                    domain_levels, 'SSTR_freq', min_count = 25, y_plabel=c(0.5, 0.7))
# 
# plot_comb_grid(indel.dt[exp_pool=='mean_RSTP2_2000_LBR2_DMSO_ssODN_64'], opt_df,
...
barcodes_all_drugs <- trip_tib_2000 %>%
  filter(exp_pool %in% c("mean_RSTP2_2000_LBR2_DMSO_ssODN_64", "mean_RSTP2_2000_LBR2_NU7441_ssODN_64")) %>% 
  group_by(barcode) %>%
  dplyr::summarise(n = n()) %>%
  filter(n == 2) %>%
  pull(barcode)

indel.dt = trip_tib %>% filter(ssODN == "ssODN") %>%
  dplyr::select(barcode, drug, SSTR, domains) %>%
  filter(barcode %in% barcodes_all_drugs) %>%
  spread(drug, SSTR) %>%
...
eff.dt = melt(indel.dt, measure.vars=c('DMSO', 'NU7441'),
              variable.name='condition', value.name='SSTR')

plot_comb_grid_boxplot(eff.dt, opt_df, domain_levels, 'SSTR', group_by='condition', min_count = 25, color=c("DMSO" = "#007B4D", "NU7441" = "#83A88B"))

groups <- unique(eff.dt$group)[c(2:13)]

for(i in groups) {
  pvals <- eff.dt %>% 
    dplyr::select(condition, SSTR, group) %>% 
    group_by(condition) %>%
    filter(group %in% c("iDomain", i)) %>%
    do(broom::tidy(wilcox.test(SSTR ~ group, data = .))) %>%
    ungroup() %>%
...
indel.dt = trip_tib %>% filter(ssODN == "ssODN") %>%
  dplyr::select(barcode, drug, SSTR_MMEJSSTR, domains) %>%
  filter(barcode %in% barcodes_all_drugs) %>%
  mutate(SSTR_MMEJSSTR = 1-SSTR_MMEJSSTR) %>%
  spread(drug, SSTR_MMEJSSTR) %>%
  distinct(barcode, .keep_all = TRUE) %>%
  data.table()
...
eff.dt = melt(indel.dt, measure.vars=c('DMSO', 'NU7441'),
              variable.name='condition', value.name='SSTR_MMEJSSTR')

plot_comb_grid_boxplot(eff.dt, opt_df, domain_levels, 'SSTR_MMEJSSTR', group_by='condition', min_count = 25, color=c("DMSO" = "black", "NU7441" = "gray"))

for(i in groups) {
  pvals <- eff.dt %>% 
    dplyr::select(condition, SSTR_MMEJSSTR, group) %>% 
    group_by(condition) %>%
    filter(group %in% c("iDomain", i)) %>%
    do(broom::tidy(wilcox.test(SSTR_MMEJSSTR ~ group, data = .))) %>%
    ungroup() %>%
    mutate(p.adj = p.adjust(p.value, method = "fdr")) %>%
    dplyr::select(condition, statistic, p.value, p.adj) %>%
    arrange(p.adj)
...
trip_lbr2_ssODN_efficiency <- trip_tib %>% dplyr::select(barcode, ssODN, SSTR, late_replicating, drug) %>% spread(drug, SSTR)

ggplot(trip_lbr2_ssODN_efficiency, aes(DMSO, NU7441, color = ssODN)) +
  geom_point() +
  theme_bw(base_size = 16) +
  geom_abline(slope = 1)  +
  geom_smooth(method = glm) +
  xlim(0,1) + ylim(0,1)  +
  labs(title = "SSTR in DMSO / NU7441 condition",
       x = "SSTR DMSO",
       y = "SSTR NU7441") +
  stat_cor(method = "pearson", label.x.npc = "left", label.y.npc = "top") +
  theme(legend.position = c(0.8, 0.2)) +
  scale_color_manual(values = c("-" = "black", "ssODN" = "gray"))
```
...
plot_comb_grid_boxplot(eff.dt, opt_df, domain_levels, 'SSTR', group_by='condition', color=c("DMSO" = "black", "NU7441" = "gray"))
```
...
plot_comb_grid_boxplot(eff.dt, opt_df, domain_levels, 'SSTR_MMEJSSTR', group_by='condition', color=c("DMSO" = "#007B4D", "NU7441" = "#83A88B"))
```
