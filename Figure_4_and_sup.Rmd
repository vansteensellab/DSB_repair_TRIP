...
## Panel 4C
```{r Fig4C MMEJ score vs eff, fig.height=4, fig.width=4}
...
ggplot(filter(trip_tib_2000, exp_pool == "mean_RSTP2_2000_LBR2_DMSO_ssODN_64" & cell_line == "RSTP2_2000"), aes(efficiency, NHEJ)) +
  geom_point() +
  geom_smooth(method= "glm") + 
  theme_bw(base_size = 16) + 
  stat_cor(method = "spearman", label.x.npc = "left", label.y.npc = "bottom")
...
## Panel 4D
```{r Fig4D pathways vs chrom, fig.height=8, fig.width=9}
chrom.mods.cols <- seq(grep("binsize", names(trip_tib))+1, grep("H3K27me3_domain", names(trip_tib))-1)

NHEJ.hist.corr <- CorrelationDFMean(trip_tib, "mean_RSTP2_2000_LBR2_DMSO_64", "NHEJ", targets = chrom.mods.cols) %>%
  arrange(corr) %>% pull(rowname) %>% factor(.,levels = .)

lims = 1
colores <- c("other_indels" = "black", "NHEJ" = "#e1251b", "MMEJ" = "#26419a")

CorrelationPlotMean(trip_tib, "mean_RSTP2_2000_LBR2_DMSO_64", c("NHEJ", "MMEJ", "other_indels") , targets = NHEJ.hist.corr)
```
...
## Panel S4A
```{r FigS5A scatterplot pathways vs lmnb1 and K4me1, fig.height=8, fig.width=12}
trip_tib %>%
...
## Panel S4B
```{r FigS5B mars bayes, fig.height=8, fig.width=8}
...
indel_chip.dt = indel_chip.dt[, -c('H3K9me3')] # removed due to terrible data sets.
