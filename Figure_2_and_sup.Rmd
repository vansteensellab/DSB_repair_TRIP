...
trip_tib_mut_2000 %>%
  ...
  facet_wrap(~ plasmid)
```
```{r Fig2B indel spectra otherguides, fig.height=8, fig.width=12}
trip_tib_mut_2000 %>%
  filter(exp_pool %in% c("mean_RSTP2_2000_LBR1_DMSO_64",
                         "mean_RSTP2_2000_LBR2_DMSO_64",
                         "mean_RSTP2_2000_LBR12_DMSO_64",
...
-
## Panel 2C
```{r Fig2C individual barcodes, fig.height=5, fig.width=7}
set.seed(4)
# set.seed(11)
...
NHEJratio <- filter(trip_tib, barcode %in% barcodeset, exp_pool == "mean_RSTP2_2000_LBR2_64") %>% 
  ...
  mutate(label_efficiency = paste0("Total indel rate = ", round(efficiency*100, 1),"%"), 
         label_ratio = paste0("MMEJ:NHEJ balance = ", round(MMEJ_MMEJNHEJ, 2)),
...
#   scale_y_continuous(name="Frequency") +
#   scale_x_discrete(name="Indel size", breaks = c("<-15", -7, 0, 1, ">3")) +
#   geom_errorbar(aes(ymin=norm-sd_norm, ymax=norm+sd_norm), width=.2,
#                  position=position_dodge(.9)) 
# p + facet_wrap( ~ label, ncol = 3, labeller = labeller(label = loc_labels))
# 
########### 
# barcodes <- trip_tib_mut %>% filter(drug == "DMSO", nexp > 5) %>% pull(barcode) %>% unique()
# barcodeset <- sample(barcodes, 6)
# 
# NHEJratio <- filter(trip_tib, barcode %in% barcodeset, exp_pool == "mean_RSTP2_2000_LBR2_DMSO_64") %>% 
#   distinct(barcode, .keep_all = T) %>% 
#   dplyr::select(barcode, NHEJ_MMEJ, efficiency) %>% 
...
# p <- ggplot(filter(trip_tib_mut_2000, barcode %in% barcodeset,
#                    drug == "-", siRNA == "-",
#                    plasmid == "LBR2", time == 64, ssODN == "-"),
#             aes(mutation, freq)) +
#     geom_bar(stat = "identity", aes(fill = color)) +
#     scale_fill_manual("legend", values = colore) +
#     theme_bw() +
...
barcodes_DMSO <- trip_tib_mut_all %>% filter(exp_pool == "mean_RSTP2_2000_LBR2_DMSO_64") %>% pull(barcode) %>% unique
barcodes_NU7441 <- trip_tib_mut_all %>% filter(exp_pool == "mean_RSTP2_2000_LBR2_NU7441_64") %>% pull(barcode) %>% unique


barcode_NU7441 <- Reduce(intersect, list(barcodes_DMSO,
                       barcodes_NU7441)) %>% unique()

trip_tib_mut_filt <- trip_tib_mut_all %>% 
  filter(barcode %in% barcode_NU7441) %>% 
  filter(exp_pool %in% c("mean_RSTP2_2000_LBR2_DMSO_64", 
...
trip_mut_lbr2_NU7441_diff <- trip_tib_mut_filt  %>%
  ...
  filter(!is.na(mean_RSTP2_2000_LBR2_NU7441_64), 
         !is.na(mean_RSTP2_2000_LBR2_DMSO_64)) %>%
...
trip_tib_mut_filt <- trip_tib_mut_all %>%
  filter(mutation %in% c("-7", "1") &
           exp_pool %in% c("mean_RSTP2_2000_LBR2_DMSO_64",
                           "mean_RSTP2_2000_LBR2_NU7441_64")) %>%
...
```
### Figure legend
*(a)* Frequency of +1 and -7 indels for all the barcodes in the pools with DMSO control (black, mean of n = 6) or the DNAPKcs inhibitor NU7441 (1uM, gray, mean of n = 5). **** = adjusted p. value < 0.0001. 

## Panel S2B
```{r FigS2B NU7441 in the pool, fig.height=6, fig.width=3}
# DMSO samples
LBR2.DMSO.64h.samples <- trip_tib_mut_all %>% filter(plasmid == "LBR2",
                                                     drug == "DMSO",
                                                     ssODN == "-",
                                                     siRNA == "-",
                                                     replicate != "mean",
...
# NU7441 LBR2 sample
LBR2.NU7441.64h.samples <- trip_tib_mut_all %>% filter(plasmid == "LBR2",
                                                       drug == "NU7441",
                                                       ssODN == "-",
                                                       siRNA == "-",
                                                       replicate != "mean",
...
barcodes_DMSO <- trip_tib_mut_all %>% filter(exp_pool %in% LBR2.DMSO.64h.samples) %>% pull(barcode) %>% unique
barcodes_NU7441 <- trip_tib_mut_all %>% filter(exp_pool %in% LBR2.NU7441.64h.samples) %>% pull(barcode) %>% unique


barcode_NU7441 <- Reduce(intersect, list(barcodes_DMSO,
                       barcodes_NU7441))

trip_tib_mut_filt <- trip_tib_mut_all %>% filter(barcode %in% barcode_NU7441) %>% filter(exp_pool %in% c(LBR2.DMSO.64h.samples, LBR2.NU7441.64h.samples))
...
### Figure legend 
*(b)* Frequency of +1 and -7 indels for the median of all the barcodes in the pools in each replicate with DMSO control (black, n = 6) or the DNAPKcs inhibitor NU7441 (1uM, gray, n = 5). * = adjusted p. value < 0.05, ** = adjusted p. value < 0.01. 
...
```{r FigS2C M3814 in the clone, fig.height=6, fig.width=4}
barcodes_clone_DMSO <- trip_tib_mut_all %>% filter(exp_pool == "mean_") %>% pull(barcode) %>% unique
barcodes_clone_M3814 <- trip_tib_mut_all %>% filter(exp_pool == "mean_RSTP2_2000_LBR2_M3814_64") %>% pull(barcode) %>% unique

barcode_clone5 <- Reduce(intersect, list(barcodes_DMSO,
                       barcodes_NU7441)) %>% unique()

trip_tib_mut_filt <- trip_tib_mut_all %>% 
  filter(barcode %in% barcode_clone5) %>% 
  filter(exp_pool %in% c("mean_RSTP2_2000_LBR2_DMSO_64", 
...
mutations <- trip_mut_lbr2_clone_M3814_diff %>% group_by(mutation) %>% dplyr::summarise(count = n()) %>% filter(count > 10) %>% pull(mutation)

trip_mut_lbr2_clone_M3814_diff %>% filter(mutation %in% mutations) %>%
  do(broom::tidy(t.test(freq ~ exp_pool, data = .))) %>%
  ungroup() %>%
  mutate(p.adj = p.adjust(p.value, method = "fdr")) %>%
 dplyr::select(mutation, p.value, p.adj) %>%
...
trip_tib_mut_filt <- trip_tib_mut_all %>%
  filter(mutation %in% c("-7", "1") &
           exp_pool %in% c("mean_RSTP2_2000_LBR2_M3814_64",
                           "mean_RSTP2_2000_LBR2_DMSO_64")) %>%
  group_by(mutation, drug)

trip_tib_mut_filt$drug <- factor(trip_tib_mut_filt$drug, levels = c("DMSO", "M3814"))
...
ggplot(trip_tib_mut_filt, aes(mutation, freq, color = drug)) +
  ...
  ylim(0, 1) +
  theme(legend.position = c(0.3, .8)) +
  scale_color_manual(values = c("DMSO" = "black", "M3814" = "gray"))

trip_tib_mut_filt %>% dplyr::summarise(mean = mean(freq))
```
### Figure legend 
*(b)* Frequency of +1 and -7 indels for each barcode in clone5 with DMSO control (black, n = 1) or the DNAPKcs inhibitor M3814 (1uM, gray, n = 1). **** = adjusted p. value < 0.0001. 

## Panel S2D
```{r FigS2D pathways with siRNA, fig.height=5, fig.width=7}
siRNAexps <- trip_tib_mut_all %>% filter(replicate %in% c('mean'), !siRNA %in% c("-", "siLigIV"), cell_line == "RSTP2_clone5", plasmid == "LBR2", time == 64) %>% pull(exp_pool) %>% unique()
siRNAexps_sep <- trip_tib_mut_all %>% filter(replicate != 'mean', !siRNA %in% c("-", "siLigIV"), cell_line == "RSTP2_clone5", plasmid == "LBR2", time == 64) %>% pull(exp_pool) %>% unique()
...
trip_mut_lbr2_siRNA_diff <- trip_mut_lbr2_all_siRNA  %>%
  ...
  spread(siRNA, freq) %>%
  na.omit() %>%
  gather(siCtIP, siNT, siPolQ, siRad51, key = "siRNA", value = "freq") %>%
  mutate(mutation = as.character(mutation)) %>% dplyr::select(-barcode, -color) %>% group_by(mutation)
...
mutations <- trip_mut_lbr2_siRNA_diff %>% group_by(mutation) %>% dplyr::summarise(count = n()) %>% filter(count > 20) %>% pull(mutation)
...
#            siRNA %in% c("siNT", i),
#            drug == "DMSO") %>%
#     do(broom::tidy(t.test(freq ~ siRNA, data = ., paired = T))) %>%
#     ungroup() %>%
#     mutate(p.adj = p.adjust(p.value, method = "fdr")) %>%
...
# 
# 
# trip_mut_lbr2_all_siRNA %>%
#   filter(mutation %in% c("-7", "1", "ssODN", "0"), drug == "DMSO") %>%
#   group_by(mutation, siRNA) %>%
#   dplyr::select(barcode, mutation, siRNA, freq) %>%
#   ggplot(., aes(siRNA, freq, color = mutation)) +
#   geom_quasirandom(dodge.width=1) +
#   theme_bw(base_size = 16) + 
...
trip_tib_mut_filt <- trip_mut_lbr2_all_siRNA %>%
  filter(mutation %in% mutations, drug == "DMSO") %>%
  group_by(mutation, siRNA) %>%
  dplyr::select(barcode, mutation, siRNA, freq) %>% 
  spread(siRNA, freq) %>%
  ...
  gather(ratio_siCtIP, 
         ...
         key = "si_ratio", 
         value = "ratio") %>% 
  dplyr::select(barcode, mutation, si_ratio, ratio)
...
ggplot(filter(trip_tib_mut_filt, mutation %in% c("-7", "1", "ssODN", "0")), aes(si_ratio, ratio, color = mutation)) +
  geom_quasirandom(dodge.width=1) +
  theme_bw(base_size = 16) + 
  scale_color_manual("legend", values = colora) +
  facet_grid(~ mutation) + 
  scale_x_discrete(labels=c("ratio_siCtIP" = "siCtIP",
                            "ratio_siPolQ" = "siPolQ",
                            "ratio_siRad51" = "siRad51")) +
  geom_text(data = repair_pvals, aes(x = si_ratio, y = 2.4 , label = p.star), size = 6)


```

### Figure legend
*(d)* Log2 ratio of the frequencies for -7 deletion, +1 insertion and ssODN integration in siRNA treatment over non-target siRNA control treatment. Adjusted p-values for target versus non-target indel frequency. *** = adjusted p. value < 0.001, **** = adjusted p. value < 0.0001. 
...
### Figure legend
*(e)* Total abundance of each pathway with the different siRNA treatments over all barcodes in clone5. Red = NHEJ (+1), blue = MMEJ (-7), green = SSTR (ssODN), black = all other indels. Left is DMSO treatment, right NU7441 treatment (1uM). 


## Panel S2D
```{r FigS2D pathways with HR siRNA, fig.height=5, fig.width=7}
siRNA_HR_exps <- trip_tib_mut_all %>% filter(replicate %in% c('mean'), drug == "Shield", siRNA %in% c("siNT", "siPolQ", "siRad51", "siBRCA1", "siBRCA2"), cell_line == "RSTP2_clone5", plasmid == "LBR2", time == 72) %>% pull(exp_pool) %>% unique()
...
trip_mut_lbr2_siRNA_HR <- trip_tib_mut_2000 %>%
  filter(exp_pool %in% c(siRNA_HR_exps))

trip_mut_lbr2_all_siRNA_HR <- trip_tib_mut_all %>%
  filter(exp_pool %in% c(siRNA_HR_exps))

trip_mut_lbr2_siHR_diff <- trip_mut_lbr2_all_siRNA_HR  %>%
  dplyr::select("barcode", "mutation", "freq", "color", "siRNA", "drug") %>%
  spread(siRNA, freq) %>%
  na.omit() %>%
  gather(siNT, siPolQ, siRad51, siBRCA1, siBRCA2, key = "siRNA", value = "freq") %>%
  mutate(mutation = as.character(mutation)) %>% dplyr::select(-barcode, -color) %>% group_by(mutation)
...
trip_mut_lbr2_siHR_diff$siRNA <- factor(trip_mut_lbr2_siHR_diff$siRNA, levels=exp_order)

mutations <- trip_mut_lbr2_siHR_diff %>% group_by(mutation) %>% dplyr::summarise(count = n()) %>% filter(count > 20) %>% pull(mutation)

trip_tib_mut_filt_HR <- trip_mut_lbr2_all_siRNA_HR %>%
  filter(mutation %in% mutations, drug == "Shield") %>%
  group_by(mutation, siRNA) %>%
  dplyr::select(barcode, mutation, siRNA, freq) %>% 
  spread(siRNA, freq) %>%
...
ggplot(filter(trip_tib_mut_filt_HR, mutation %in% c("-7", "1", "0")), aes(si_ratio, ratio, color = mutation)) +
  geom_quasirandom(dodge.width=1) +
  theme_bw(base_size = 16) + 
  scale_color_manual("legend", values = colora) +
  facet_wrap(~ mutation) + 
...
trip_lbr2_siRNA_HR %>% 
  ...
  mutate(MMEJandNHEJ = MMEJ+NHEJ, HR_Balance = efficiency/MMEJandNHEJ) %>% 
  filter(replicate == "mean") %>% 
  ggplot(., aes(MMEJ_MMEJNHEJ, HR_Balance, color = siRNA)) + 
  geom_point()

```

```{r FigS2E pathway contribution with HR siRNA, fig.height=5, fig.width=6}
