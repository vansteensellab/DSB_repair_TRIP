...
### Panel B
```{r indel spectra, fig.height=6, fig.width=10}
...
## Supplementary 1
### Panel A
```{r chromideograms supp, fig.height=11, fig.width=6}
ggplot() +
  geom_segment(data = chromsizes,
               aes(x = chr_r, xend = chr_r, y = 0, yend = pos_r),
               lineend = "round", color = "lightgrey", size = 7) +
...
### Panel C
<!-- start Christ -->
```{r clone5 heatmap Christ, fig.height=6, fig.width=8}
...
trip_tib_mut_all_selection %>% filter(mutation != "ssODN", drug == "-") %>% group_by(mutation) %>% dplyr::summarise(median = log10(median(freq))) %>%
  ggplot(., aes(mutation, fill = median, width=1)) + geom_bar(stat = "count") + theme_bw(base_size = 16) + scale_fill_viridis()
```
...
barcodes_DMSO <- trip_tib_mut_all %>% filter(exp_pool == "indel_mean_LBR2_DMSO_64h") %>% pull(barcode) %>% unique
barcodes_NU7441 <- trip_tib_mut_all %>% filter(exp_pool == "indel_mean_LBR2_NU7441_64h") %>% pull(barcode) %>% unique
...
trip_mut_lbr2_NU7441_diff <- trip_tib_mut_filt  %>%
  ...
  filter(!is.na(indel_mean_LBR2_NU7441_64h), 
         !is.na(indel_mean_LBR2_DMSO_64h)) %>%
  gather(indel_mean_LBR2_NU7441_64h, 
         indel_mean_LBR2_DMSO_64h, 
...
trip_tib_mut_filt <- trip_mut_lbr2_all_siRNA %>%
  filter(mutation %in% c("-22", "-14", "-7", "1", "ssODN")) %>%
  group_by(mutation, drug, siRNA)

ggplot(trip_tib_mut_filt, aes(mutation, freq, color = siRNA)) +
  geom_quasirandom(dodge.width=1) +
  theme_bw(base_size = 16) +
  facet_wrap(~ drug)
```
