...
load(paste0(in.dir, "total_indel_data.RData"))
load(paste0(in.dir, "Analyis_Mapping_RSTP2_2000.RData"))
load(paste0(in.dir, "mutation_frequencies_trunc.RData"))
...
ChIP_meta <- readRDS("/DATA/projects/DSBrepair/data/R/cl20191219_ChIP_meta_selection_chipmentation.RDS")
...
damain.files <- damain.files[grep("r[12]", names(damain.files), invert = TRUE)]
damain.files <- damain.files[names(damain.files) %in% ChIP_meta$chip_name]
...
ChIP_mean_data[pool == "clone9", ID := paste0(ID, ".B")]
...
file.dt = data.table(str_match(file_list, '.*/20180918.RSTP2_(.*)-r([0-9]).txt'))
names(file.dt) = c('file', 'target', 'rep')

rep.dt = file.dt[!is.na(file),
                 fread(file, col.names=c('barcode', 'domain', 'distance')),
				 by=c('target', 'rep')]

rep.dt[target=='repliseq', domain:=ifelse(domain=='iDomain', 'Domain', 'iDomain')]

rep.dt[target=='repliseq',target:='late_replicating']

chip.domains.dt = rep.dt[,list(domain=ifelse(all(domain=='Domain') & sum(distance)==0,
                                        'Domain', 'iDomain')),
                    by=c('target', 'barcode')]
...
spin.domains.dt = spin.dt[,list(domain=ifelse(all(domain=='Domain') & sum(distance)==0,
                                        'Domain', 'iDomain')),
                    by=c('target', 'barcode')]

# Ruben addition
## temp add clone barcodes
clone_file_list = list.files(near_dir, pattern='clone[59]', full.names=T)

clone_file.dt = data.table(str_match(clone_file_list, '.*/clone(.*)-r([0-9]).txt'))
names(clone_file.dt) = c('file', 'target', 'rep')
...
clone_chip.domains.dt = clone_rep.dt[,list(domain=ifelse(all(domain=='Domain') & sum(distance)==0,
                                                         'Domain', 'iDomain')),
                                     by=c('target', 'barcode')]

chip.domains.dt = rbind(clone_chip.domains.dt, chip.domains.dt, spin.domains.dt) %>% distinct(barcode, target, .keep_all = TRUE)

chip.domains.dt = mutate(chip.domains.dt, target = paste0(target, "_domain"))
chip.domains.cast.dt = spread(chip.domains.dt, target, domain)
```
...
trip_tib = left_join(tib_RSTP2_2000_mutations, chip_domains.dt, by = c("barcode", "cell_line" = "pool")) %>%
  as_tibble()
...
trip_tib_mut = left_join(all.mutations.trunc.dt, chip_domains.dt, by = c("barcode", "cell_line" = "pool")) %>% as_tibble()
...
save(trip_tib_2000, file = paste0(out.dir, "RSTP2_IndelRatios_Chromatin_2kb.RData"))

# Mutations truncated
# save(trip_tib_mut_1000, file = paste0(out.dir, "RSTP2_Indel_Chromatin_1kb.RData"))
save(trip_tib_mut_2000, file = paste0(out.dir, "RSTP2_Indel_Chromatin_2kb.RData"))
```
