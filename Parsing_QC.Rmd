...
## Select outdir
out.dir = "/DATA/projects/DSBrepair/data/R/rs20191220/"
dir.create(out.dir)
```
...
file.list <- list.files(path = "/DATA/projects/DSBrepair/data/rs20190716_TRIP/parsed/", pattern="mapping*.*statistics.txt", full.names = TRUE)
...
file.list <- list.files(path = "/DATA/projects/DSBrepair/data/rs20190716_TRIP/table/", pattern="*.table", full.names = TRUE)
...
# Load metadata
metadata <- read.table("/DATA/projects/DSBrepair/config/rs20190708_metadata_Repair_TRIP.tsv", header = TRUE, stringsAsFactors = FALSE) %>% as_tibble()
```
...
filtered_mapping.df <- filtered_mapping.df %>% select(-mapq_sum2, -reads2, -mapq_2, -freq2) # Remove mapq_sum, reads2, mapq2, freq2
...
table(dupli.vec.test %in% 
        filtered_mapping.df[filtered_mapping.df$barcode %in% 
                                                bothpools.vec, ]$start_pos)

filtered_mapping.df %>% filter(start_pos %in% dupli.vec.test[!dupli.vec.test %in% 
        filtered_mapping.df[filtered_mapping.df$barcode %in% 
                                                bothpools.vec, ]$start_pos])

```

Yes, the data looks good like this. There are only 3 reads that are mapped exactly to the same location, two in A & B with different barcodes, and one with similar looking BC only in A due to two differnt sequences (maybe a snp at the GAT[A/C] where the sequence is cut).
...
filename <- SetFileName("_Mapping_RSTP2_2000.bed", "rs")
...
filename <- SetFileName("_Analyis_Mapping_RSTP2_2000.txt", "rs")
...
filename <- SetFileName("_Analyis_Mapping_RSTP2_2000.RData", "rs")
