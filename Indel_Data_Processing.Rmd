...
file.list <- list.files("/DATA/projects/DSBrepair/data/rs20200221_TRIP/indelPCR_counts/",
...
# Read metadata file
metadata <- read.table("/DATA/projects/DSBrepair/config/rs20200829_metadata_Repair_TRIP.txt", 
                       header = TRUE, 
                       stringsAsFactors = FALSE) %>% 
...
nonmappedmutations.dt = mutations.dt[!barcode %in% mapped_barcodes]
dim(mutations.dt)
dim(nonmappedmutations.dt)
mutations.dt = mutations.dt[barcode %in% mapped_barcodes]
dim(mutations.dt)

# Fill up the big data table to have 0 counts also, this will be easier to count the means and sds later on.
complete.mutations.dt = mutations.dt %>% complete(mutation, nesting(exp, barcode), fill = list(count = 0)) %>% data.table()
dim(complete.mutations.dt)
sum(complete.mutations.dt$count)

# Merge metadata with the mutation data table.
mutationsmeta.dt = complete.mutations.dt[metadata,on = c("exp" = "ID")]
...
filtered.mutationsmeta.dt <- mutationsmeta.dt %>%
  ...
  data.table()
dim(filtered.mutationsmeta.dt)

# Some demultiplexing issues with short PCR (interpreted as -81/82 deletions) that were only from clone5 or 9.
filtered.mutationsmeta.dt <- filtered.mutationsmeta.dt[!(barcode %in% c(clone5bc, 
                                                      clone9bc, 
                                                      clone5barcodes) & 
                                         replicate %in% c("LBR2GSKBIXRep2", 
                                                          "LBR2ssODNRep3", 
                                                          "clonesiRNARep1", 
                                                          "clonesiRNARep2") & 
...
filtered.mutationsmeta.dt <- filtered.mutationsmeta.dt[!(barcode %in% c(clone5bc, 
                                                      clone5barcodes) & 
                                         replicate %in% c("NewGuidesM3814Rep1", 
                                                          "NewGuidesM3814Rep2", 
                                                          "LBR2M3814Rep1", 
                                                          "LBR2M3814Rep2")
                                       )
...
bc_count.dt = filtered.mutationsmeta.dt[, list(sum_bc_reads = sum(count)), by=c("exp", 'barcode')]

# We used approximately 600,000 cells per experiment in the PCRs (by amount of gDNA used)
bc_count.dt = bc_count.dt[, cells:=(sum_bc_reads/sum(sum_bc_reads))*600000, by='exp']
filtered.mutations.dt = filtered.mutationsmeta.dt[bc_count.dt[cells > 50, ], on = c("exp", "barcode")]

dim(filtered.mutations.dt)

filtered.mutations.dt[, norm:=count/sum(count), by=exp,]
```
## Calculate frequencies

```{r}
filtered.mutations.dt[,freq := norm/sum(norm), 
                 by = c("barcode","exp")]

filtered.mutations.dt[!mutation%in%c(0,NA), 
...
```{r mean replicates}
map.meta.mutations.dt = copy(filtered.mutations.dt)
...
sep.mutations.dt = map.meta.mutations.dt[, c("group", "cells_tot", "mean_bc_reads", "sd_norm", "sd_freq", "sd_ratio_indels", "nexp") :=
                        list(NULL, NA, NA, NA, NA, NA, 1)]
...
gsk.mutations.dt = map.meta.mutations.dt[replicate %in% c("LBR2GSKBIXRep1", "LBR2GSKBIXRep2", "LBR2GSKBIXRep3") & drug == "DMSO", ]
...
gsk.mutations.c5.dt = map.meta.mutations.dt[replicate %in% c("Clone5GSKBIXRep1", "Clone5GSKBIXRep2", "Clone5GSKBIXRep3") & drug == "DMSO", ]
...
all.mutations.dt <- do.call(rbind, list(sep.mutations.dt, mean.mutations.dt, mean.bixgsk.dt, mean.bixgsk.c5.dt))
```
...
setcolorder(all.mutations.dt, c("exp", "exp_pool", "barcode", "nexp", "no",
                               ...
                               "sum_reads", "mean_bc_reads", "sum_bc_reads",
                               "cells", "cells_tot", "mutation", "count", "norm",
...
setcolorder(large.indels.dt,c("exp", "exp_pool", "barcode", "nexp", "no",
                               "replicate", "cell_line", "pool", "plasmid", "time", "drug",
                               "ssODN", "siRNA", "GCF","seq_barcode","seq_index", 
                               "sum_reads", "mean_bc_reads", "sum_bc_reads",
                               "cells", "cells_tot", "mutation", "count", "norm",
