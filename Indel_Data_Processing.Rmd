...
# Read metadata file
metadata <- read.table("/DATA/projects/DSBrepair/config/rs20200226_metadata_Repair_TRIP.txt", 
                       header = TRUE, 
                       stringsAsFactors = FALSE) %>% 
...
mut.list = mclapply(names(df.list), function(exp){
  ...
  pool = gsub("indel_.*_([AB]|clone5|clones[AB])_.*", "\\1", exp)
...
mutationsmeta.dt <- mutationsmeta.dt[!(barcode %in% c(clone5bc, 
                                                      clone9bc, 
                                                      clone5barcodes) & 
...
all.mutations.dt <- do.call(rbind, list(sep.mutations.dt, mean.mutations.dt, mean.bixgsk.dt))
```
