---
title: "Figure 3 and supplemental 3-4"
author: "Ruben Schep"
...
plot_comb_grid_boxplot <- function(bix.dt, opt_df, domains, y_name, min_count=0,
  ...
  #                  aes_string(x='group', y=y_name, color=group_by)) +
  #     geom_boxplot(dodge.width=1, outlier.shape = NA) +
...
ChIP_clone5 <- readRDS("/DATA/projects/DSBrepair/data/R/xv20200402_chipampcorrelation.rds")
...
## Panel S3A
```{r FigS3A indelspectra other guides, fig.height=10, fig.width=4}
...
res = trip_tib %>% dplyr::select(barcode, plasmid, efficiency) %>% 
  spread(plasmid, efficiency) %>% 
...
## Panel S3C
```{r FigS3C correlation TIF all guides shuffled data, fig.height=5, fig.width=5}
# Same but shuffled data. 
res_shuffle = trip_tib %>% dplyr::select(barcode, plasmid, efficiency) %>% 
  spread(plasmid, efficiency) %>% 
  column_to_rownames(., var = "barcode") %>%
  mutate(LBR2 = sample(LBR2),
         LBR1 = sample(LBR1),
         LBR12 = sample(LBR12),
         LBR15 = sample(LBR15)) %>%
  as.matrix() %>%
...
corrplot(res_shuffle$r, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45, diag = FALSE)
```
...
signal.amp.seq <- left_join(selected.chip.tib,Chip.seq.data.melt, by = c("barcode", "antibody")) %>% left_join(clone5_domains, by = "barcode")

#Plot with free axis scales
ggplot(signal.amp.seq, aes(value,amp.seq)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  facet_wrap(~ antibody, scales= "free") + 
  ylab("log2(normalized barcode reads)") + 
  xlab("log2(ChIP-seq signal)") + 
  stat_cor(method = "pearson", label.x.npc = 0, label.y.npc = 1, vjust = 0) +
  theme_bw(base_size = 16)
...
<!-- indel.dt = trip_tib %>% -->
<!--   dplyr::select(barcode, plasmid, efficiency, domains) %>% -->
<!--   filter(barcode %in% barcodes_all_plasmids) %>% -->
<!--   spread(plasmid, efficiency) %>% -->
<!--   distinct(barcode, .keep_all = TRUE) %>% -->
<!--   data.table() -->
...
<!-- eff.dt = melt(indel.dt, measure.vars=c('LBR1', 'LBR12', 'LBR15', 'LBR2'), -->
<!--               variable.name='condition', value.name='efficiency') -->

<!-- plot_comb_grid_boxplot(eff.dt, opt_df, domain_levels, 'efficiency', group_by='condition', color=guide_colors) -->
